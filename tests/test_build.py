...
def test_build(client: DockerClient) -> None:
    ...
    assert "COPY ./entrypoint.sh /usr/local/bin/" in logs_str
    assert (
    ...
    assert ('CMD [ "/usr/local/bin/entrypoint.sh" ]') in logs_str
    assert image
    ...
    assert image.attrs.get("Config").get("Cmd") == ["/usr/local/bin/entrypoint.sh"]
